...
```{r, eval=F}
out <- all1 %>%
  select(date, ethnicity, agec, CAAP, RSV, influenza.a, influenza.b,parainfluenza, respiratory.adenovirus)
write.csv(out, './DONOTSYNC/processed_ts.csv')
```
...
```{r}
obs <- all1 %>%
  select(date, ethnicity, agec,CAAP, nonCAAP,bacteremic.pneumonia ,other.IPD) %>%
  group_by(agec, date) %>%
  summarize(CAAP=sum(CAAP), nonCAAP=sum(nonCAAP),bacteremic.pneumonia=sum(bacteremic.pneumonia) ,other.IPD=sum(other.IPD) ) %>%
  ungroup() %>%
  reshape2::melt(.,id.vars=c('agec', 'date')) %>%
  rename(Obs=value ,outcome=variable) %>%
  mutate(agec=as.numeric(agec))

all.preds %>%
  left_join(obs, by=c('agec','date', 'outcome')) %>%
  ggplot( aes(x=date, y=pred)) +
  geom_line()+
  geom_line(aes(x=date, y=Obs),color='gray')+
    theme_minimal()+
  facet_grid(outcome~ agec, scales='free_y')

```

## Overall obs vs expected


```{r}
mod.results <- readRDS('./Results/model_fits_poisson.rds')

preds.caap.all <-  mod.results$res1.caap.all$overall.preds$fitted %>%
  as.data.frame() %>%
  mutate(outcome='CAAP',time= row_number(),date= as.Date('2016-01-01') + months(time-1))

preds.non.caap.all <-  mod.results$res1.noncaap.all$overall.preds$fitted %>%
    as.data.frame() %>%
  mutate(outcome='nonCAAP',time= row_number(),date= as.Date('2016-01-01') + months(time-1))

preds.non.ipd.pneu.all <-  mod.results$res1.ipd.pneu.all$overall.preds$fitted %>%
  as.data.frame() %>%
    mutate(outcome='bacteremic.pneumonia',time= row_number(),date= as.Date('2016-01-01') + months(time-1))

preds.non.ipd.non.pneu.all <-  mod.results$res1.ipd.other.all$overall.preds$fitted %>%
  as.data.frame() %>%
    mutate(outcome='other.IPD',time= row_number(),date= as.Date('2016-01-01') + months(time-1))

all.preds.overall <- bind_rows(preds.caap.all,preds.non.caap.all,preds.non.ipd.pneu.all,preds.non.ipd.non.pneu.all) 
write.csv(all.preds.overall,'Results/pred2022.overall.csv')

ggplot(all.preds.overall, aes(x=date, y=med.pred)) +
  geom_line()+
  theme_minimal()+
  ylim(0,NA)+
