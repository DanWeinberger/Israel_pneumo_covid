...
```{r}
#all1.spl <- split(all1,paste(all1$agec, all1$ethnicity) )
#res1 <- pblapply(all1.spl, inla_model2, outcome.var='caap.pre', plot.var='CAAP')

res1 <- inla_model2(all1,  outcome.var='caap.pre', plot.var='CAAP')

```

```{r}

preds_time_all <- res1$all$preds_time

preds_time_NoVirus <- res1$NoVirus$preds_time
...
ggplot(preds_time_all, aes(x=date, y=CAAP)) +
...
#Constraints-- pred can't be <0, prednovirus can't be > pred
# preds.iter2$pred[preds.iter2$pred<0] <- 0
# preds.iter2$pred_NoVirus[preds.iter2$pred_NoVirus<0] <- 0


# preds.iter2$pred_NoVirus[preds.iter2$pred_NoVirus>preds.iter2$pred] <-
#   preds.iter2$pred[preds.iter2$pred_NoVirus>preds.iter2$pred]


#let's create a flag for any iterations where there is a negative value predicted or where founterfact is > pred

flags <- preds.iter2 %>%
  group_by( variable) %>%
  mutate(min_pred= min(pred) , flag_min_pred=(min_pred<0),
         )

attrib_pct <- flags[flags$flag_min_pred==0,] %>%
  group_by( variable) %>%
